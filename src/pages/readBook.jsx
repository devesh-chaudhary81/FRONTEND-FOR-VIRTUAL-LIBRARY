...
const ReadBook = ({ userId }) => {
  ...
  try {
    ...
    setSummary(res.data.summary || res.data.quiz);
  } catch (err) {
...
//   const [isGeneratingQuiz, setIsGeneratingQuiz] = useState(false); // Separate for quiz
//   const [isGeneratingNotes, setIsGeneratingNotes] = useState(false); // Separate for notes
...
//     try {
//       const res = await axios.post("http://localhost:3000/api/books/Notes-by-range", {
//         pdfUrl: bookURL,
//         startPage: range.start,
//         endPage: range.end,
//       });
...
//         <h2 className="text-lg font-semibold mb-2 text-black">📘 Generate Content</h2>
...
//               isGeneratingQuiz ? "opacity-70 cursor-not-allowed" : ""
//             }`}
//           >
...
//             className={`flex items-center justify-center gap-2 bg-green-600 text-white px-3 py-2 rounded hover:bg-green-700 transition ${
//               isGeneratingNotes ? "opacity-70 cursor-not-allowed" : ""
//             }`}
//           >
